...
def qimage_argb32_from_png_decoding(img_data):
    '''Effectue le décodage d'un 'buffer' de données correspondant à une 
       ...
       En sortie on obtient une image (instance de la class QImage) de 
    ...
    raise ValueError("Erreur de décodage d'une image avec la fonction _png_decoding.")
    return image
...
def ndarray_from_qimage_argb32(img):
    '''Effectue la conversion d'une image de format 
       ...
       L'image de sortie est convertie en une matrice de la même taille 
       ayant des valeurs 0-1 en format uint8.'''
